...
{
	global $icmsConfig;
	if(file_exists(ICMS_ROOT_PATH.'/modules/'.basename( dirname( dirname( __FILE__ ) ) ).'/language/'.$icmsConfig['language'].'/modinfo.php')) {
		include_once ICMS_ROOT_PATH.'/modules/'.basename( dirname( dirname( __FILE__ ) ) ).'/language/'.$icmsConfig['language'].'/modinfo.php';
	} else {
		include_once ICMS_ROOT_PATH.'/modules/'.basename( dirname( dirname( __FILE__ ) ) ).'/language/english/modinfo.php';
	}
	...
	echo "<a href=\"../index.php\">"._AM_EXTGALLERY_GO_TO_MODULE."</a> | <a href=\"".ICMS_URL."/modules/system/admin.php?fct=preferences&amp;op=showmod&amp;mod=".icms::$module->getVar('mid')."\">"._AM_EXTGALLERY_PREFERENCES."</a>\n";
	echo "</td>\n";
	echo "<td style=\"width: 30%; font-size: 10px; text-align: right; color: #2F5376; padding: 0 6px; line-height: 18px;\">\n";
	echo "<b>".icms::$module->getVar('name')."&nbsp;"._AM_EXTGALLERY_ADMINISTRATION."</b>&nbsp;".$breadcrumb."\n";
	echo "</td>\n";
	echo "</tr>\n";
	echo "</table>\n";
	echo "</div>\n";
	...
	foreach($GLOBALS['adminmenu'] as $key=>$link) {
		...
		echo "<a href=\"".ICMS_URL."/modules/".basename( dirname( dirname( __FILE__ ) ) )."/".$link['link']."\"><span>".$link['title']."</span></a>\n";
		echo "</li>\n";
	}
	echo "</ul>\n";
	echo "</div>\n";
