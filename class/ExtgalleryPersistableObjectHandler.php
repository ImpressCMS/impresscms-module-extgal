...
class ExtgalleryPersistableObjectHandler extends icms_core_ObjectHandler {
				...
				foreach($externalKeys as $key) {
					...
					if($ret[$i][$key] != 0) {
						...
						if(!isset($cached[$externalKey['keyName']][$ret[$i][$key]])) {
							if($externalKey['core']) {
								$handler = icms::handler($externalKey['className']);
							} else {
								$handler = icms_getModuleHandler($externalKey['className'], 'extgallery');
							}
