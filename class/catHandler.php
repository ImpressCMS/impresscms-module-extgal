...
class ExtgalleryCatHandler extends ExtgalleryPersistableObjectHandler {
	...
	function ExtgalleryCatHandler(&$db, $type)
	{
		...
		$this->_photoHandler = icms_getModuleHandler($type.'photo', 'extgallery');
	}
	...
	function getDescendants($id = 0, $includeSelf = false, $childrenOnly = false, $withRestrict = true, $permType = "public_access") {
		...
		if ($childrenOnly) {
			$criteria->add(new icms_db_criteria_Item('cat_pid',$parent_id),'OR');
			if($includeSelf) {
				$criteria->add(new icms_db_criteria_Item('cat_id',$parent_id));
			...
			if($nleft > 0 && $includeSelf) {
				$criteria->add(new icms_db_criteria_Item('nleft',$nleft,'>='));
				$criteria->add(new icms_db_criteria_Item('nright',$nright,'<='));
	...
	function getCat($id = 0) {
		$criteria = new icms_db_criteria_Compo();
		$criteria->add($this->getCatRestrictCriteria('public_displayed'));
		$criteria->add(new icms_db_criteria_Item('cat_id', $id));
	...
	function nbAlbum($id = 0) {
		$criteria = new icms_db_criteria_Compo(new icms_db_criteria_Item('nright - nleft',1));
	...
	*/
	...
	function &getCatRestrictCriteria($permType = "public_access") {
		...
		$allowedCategories = $permHandler->getAuthorizedPublicCat(icms::$user, $permType);
